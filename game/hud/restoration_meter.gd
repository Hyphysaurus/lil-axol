...
## mini-gauges track the sim's actual recovery stages: they smooth a SEPARATE cleanliness
## value through the same envelopes cove_life.gd animates with (kelp: 0→35% of the heal,
## fish: 15→55%), so the minis can never disagree with the actual kelp/fish in the water
## even where health diverges from cleanliness (estuary). Four small pips under the bar show
## the restoration engine's variables (purity/oxygen/clarity/vegetation), dimmed when a
## variable isn't in the reach's in-play set. Code-drawn, self-wired to the oil_manager +
...
var _shown_clean := 0.0        # smoothed cleanliness — the kelp/fish minis' own value
...
func _process(delta: float) -> void:
	...
	_shown_clean = move_toward(_shown_clean, _clean, delta * 0.5)   # the minis' honest source
	_pulse = maxf(0.0, _pulse - delta * 0.8)
	_gauge.set_state(_shown, _shown_clean, _pulse, _pulse_at)
...
class Gauge extends Control:
	...
	var _v := 0.0                              # main bar: reach health (or legacy cleanliness)
	var _clean_v := 0.0                        # kelp/fish minis: cleanliness ONLY (world-honest)
	var _pulse := 0.0
	var _pulse_at := 0.0
	...
	func set_state(v: float, clean_v: float, pulse: float, pulse_at: float) -> void:
		if not (is_equal_approx(_v, v) and is_equal_approx(_clean_v, clean_v)
				and is_equal_approx(_pulse, pulse)):
			_v = v
			_clean_v = clean_v
			_pulse = pulse
			_pulse_at = pulse_at
	...
	func _draw() -> void:
		...
		# stage minis: CLEANLINESS through cove_life's envelopes so meter == world (never health)
		var kelp := smoothstep(0.0, 0.35, _clean_v)
		var fish := smoothstep(0.15, 0.55, _clean_v)
